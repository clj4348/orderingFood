...
class Header extends React.Component {
  ...
  renderCateInnerContent(items, cateList){
    return items.sub_category_list.map((item, index) => {
      ...
      return (
        <div onClick={()=> this.changeDofilter(item, TYPE_KEY.cate, cateList)} key={index} className="cate-box">
          <div className={cls}>
      ...
      return (
        <li key={index} className="cate-item">
          <p className="item-title">
            {item.name}<span className="item-count">{item.quantity}</span>
          </p>
  ...
  renderFilterInnerContent(items, filterList){
    return items.map((item, index) => {
      ...
      return (
        <div onClick={()=> this.changeDofilter(item, TYPE_KEY.filter, filterList)} key={index} className="cate-box">
          <div className={cls}>
  ...
  renderTypeContent(){
    ...
    return typeList.map((item, index) => {
      ...
      return (
        <li onClick={()=> this.changeDofilter(item, TYPE_KEY.type, typeList)} key={index} className={cls}>
          {item.name}
        </li>
      )
    })
  ...
  renderContent(){
    ...
    for (let key in tabs ){
      ...
      else if(item.key === TYPE_KEY.type){
        array.push(
          <ul key={item.key} className={cls}>
